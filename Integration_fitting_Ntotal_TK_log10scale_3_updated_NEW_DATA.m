...
clearvars -except Kg N0 Kd y1
clc
dataset_1 = xlsread('BacterioScan CAZ+AMK AB747 (5 log) raw data 0222_AKIS.xlsx','Data','AQ3:AW23'); %updated data set 
X = dataset_1;
%BacterioScan CAZ+AMK AB747 (5 log) raw data 0222_AKIS


dataset(3,:) = {X(1:12,3)};
time_TK_1(3,:) = {X(1:12,1)};

dataset(4,:) = {X(1:8,4)};
time_TK_1(4,:) = {X(1:8,1)};
AKIS = 25;
dataset(5,:) = {X(1:7,5)}; %Choise was based until when we had true values nothing else matters 
time_TK_1(5,:) = {X(1:7,1)};

dataset(6,:) = {X(1:6,6)}; %Choise was based until when we had true values nothing else matters 
time_TK_1(6,:) = {X(1:6,1)};

dataset(7,:) = {X(1:8,7)}; %Choise was based until when we had true values nothing else matters BUT CAN BE CHANGED DUE TO EXPECTED LINE PLATEAU
time_TK_1(7,:) = {X(1:8,1)};
...
% options = [options_1 options_1 options_1 options_1 options_1]; %My opinion is that option 4 must be applied to C = 64 and 256 the fitting may not be as accurate but the final D?kg results are 
 options = [options_3 options_3 options_4 options_1 options_1];
    

xq1 = [6.121403527	6.072776203	6.154033559	6.02196923	6.162064308];%[6.063580125, 6.05374236, 6.198805193,6.263234182,6.306644761]; change based on time 0 
...
%                 lb = [0.000000000001 0.000000000001 0.000000000001 xq1(i) xq2(i)]; % This is the correct boundary to use
    ...
    ub = [10 10 10 10^8 10^8]; %upper bound of estiamted paramters
    ...
    C = (jacobian(:,1:5)'*jacobian(:,1:5))\eye(5); %%% same if I use inv
    
    COVARIANCE = (jacobian(:,1:5)'*jacobian(:,1:5))\eye(5)*s^2
